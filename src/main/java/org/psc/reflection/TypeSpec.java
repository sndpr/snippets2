...
public class TypeSpec<T> {
    ...
    private TypeSpec(Class<T> type, String delimiter, String nullReplacement,
        ...
        fieldResolutionSpecs = Arrays.stream(type.getDeclaredFields())
                .map(it -> methodsByFieldName.get(it.getName()))
                .filter(Objects::nonNull)
                .filter(it -> it.getter != null)
                .map(it -> new FieldResolutionSpec<>(it.field().getName(), it.field().getType(),
    ...
    public T deserialize(String value) {
        ...
        if (resolvedAllArgsConstructor != null) {
            Object[] mappedConstructorArgs = IntStream.range(0, split.length)
                    .mapToObj(i -> {
                        FieldResolutionSpec<T> currentFieldResolutionSpec = fieldResolutionSpecs.get(i);
                        String valueToDeserialize = split[i];
            ...
            IntStream.range(0, split.length)
                    .forEach(i -> {
                        ...
                        Object deserializedValue =
                                currentFieldResolutionSpec.fromStringMapper().apply(valueToDeserialize);
                        try {
    ...
    private BiFunction<T, Object, String> resolveToStringMapper(GetterAndSetter getterAndSetter) {
        ...
        if (serializationFieldMappings.containsKey(fieldName)) {
            ...
            //noinspection unchecked
            return (typeObject, fieldValue) -> ((Function<Object, String>) serializationTypeMappings
                    .entrySet()
                    .stream()
                    .filter(entry -> fieldType.equals(entry.getKey()))
                    .findFirst()
                    .orElseGet(() -> serializationTypeMappings
                            .entrySet()
                            .stream()
                            .filter(entry -> entry.getKey().isInstance(fieldType))
                            .findFirst()
    ...
    public static class Builder<T> {
        ...
        public TypeSpec<T> build() {
            return new TypeSpec<>(type, delimiter, nullReplacement,
                    serializationBuilder.fieldMappings, serializationBuilder.typeMappings,
                    deserializationBuilder.fieldMappings, deserializationBuilder.typeMappings);
        }
    ...
    public static class SerializationBuilder<T> {
        ...
        private final Map<Class<?>, Function<?, String>> typeMappings = new ConcurrentHashMap<>(
                DEFAULT_SERIALIZATION_TYPE_MAPPINGS);
